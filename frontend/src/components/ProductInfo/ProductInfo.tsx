...
const ProductInfo: React.FC<ProductInfoProps> = ({ condition, sold, title, price }): JSX.Element => {
    ...
    return (
        <section className='product-info__content'>
            <h5 className='product-info__condition'>{`${properCondition(condition)} - ${(sold.toLocaleString('es-AR'))} vendidos`}</h5>
            ...
            <Button handleOnClick={addToCart} text='Comprar' size={ButtonSizes.fullwidth}></Button>
        </section>
    )
}
